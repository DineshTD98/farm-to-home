...
import { updateProduct, getFarmerProducts } from '../../api/products';
...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const products = await getFarmerProducts(user.id || user._id);
        const product = products.find(p => p._id === id);
        if (product) {
          const isCommon = Object.values(productData).flat().includes(product.name);
          setShowCustomInput(!isCommon && !!product.name);
          ...
          setFormData({
            ...
            pricePerUnit: product.pricePerUnit || product.pricePerItem,
            description: product.description || '',
          });
          setPreview(product.image?.startsWith('http') ? product.image : `${UPLOADS_BASE}${product.image?.replace('/uploads', '')}`);
        } else {
    ...
    if (user.id || user._id) fetchProduct();
  }, [id, user.id, user._id]);
  ...
  if (fetching) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-700 transition-colors duration-500">
        <div className="w-16 h-16 border-4 border-green-50 dark:border-green-900/20 border-t-green-600 rounded-full animate-spin mb-8 shadow-inner shadow-green-900/5"></div>
        <div className="text-gray-400 dark:text-gray-500 font-black tracking-[0.5em] text-[10px] uppercase animate-pulse">{t('common.loading')}</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden font-sans transition-colors duration-500 text-gray-900 dark:text-white">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-[#111111] border border-gray-100 dark:border-white/5 rounded-[4rem] p-12 shadow-2xl shadow-gray-900/5 animate-in fade-in zoom-in duration-500 transition-colors duration-500">
        <div className="text-center mb-12 relative z-10">
          ...
          <h1 className="text-4xl font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none mb-3">{t('farmerCrops.editProduct')}</h1>
          <p className="text-gray-400 dark:text-gray-500 text-sm font-medium italic">{t('farmerCrops.editSubtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {!showCustomInput ? (
                ...
                <div className="relative group">
                  <input 
                    ...
                    onBlur={() => { 
                      const isCommon = Object.values(productData).flat().includes(formData.name);
                      if (!formData.name || isCommon) setShowCustomInput(false); 
                    }}
                    autoFocus
                    required 
            ...
            <div>
              ...
              <input type="number" name="quantity" step="0.01" value={formData.quantity} onChange={handleChange} required className={inputClass} />
            </div>
            <div>
            ...
            <div className="mt-2">
              {preview ? (
                <div className="w-full h-64 rounded-[2.5rem] overflow-hidden border border-gray-100 dark:border-white/10 group relative shadow-md">
                  ...
                  <label className="absolute inset-0 bg-white/40 dark:bg-black/40 backdrop-blur-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all font-black text-[10px] text-gray-900 dark:text-white uppercase tracking-widest cursor-pointer">
                    Change Visualization
                    <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                  </label>
                </div>
              ) : (
                <label className="w-full h-64 rounded-[2.5rem] border-2 border-dashed border-gray-100 dark:border-white/10 flex flex-col items-center justify-center cursor-pointer hover:border-green-600/40 dark:hover:border-green-500/20 hover:bg-green-50/20 dark:hover:bg-white/5 transition-all duration-300 group">
                  <div className="text-6xl mb-4 opacity-20 transform transition-transform group-hover:scale-110 duration-500">📸</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500 uppercase font-black tracking-widest">Capture Fresh Glance</div>
                  <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                </label>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            ...
            >
              {loading ? t('common.loading') : t('farmerCrops.buttons.update')}
            </button>
          </div>
        </form>
      </div>
