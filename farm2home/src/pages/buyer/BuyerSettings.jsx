...
const InputField = ({ label, ...props }) => (
  <div>
    ...
    <input
      className="w-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl px-5 py-3.5 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-[#fbbc05] focus:ring-4 focus:ring-amber-500/5 transition-all font-medium text-sm shadow-inner"
      {...props}
    />
  </div>
);
...
const BuyerSettings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Consumer profile updated.' });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] flex flex-col font-sans transition-colors duration-500 text-gray-900 dark:text-white">
      <nav className="flex items-center justify-between px-8 py-4 bg-white dark:bg-[#111111] border-b border-gray-100 dark:border-white/5 sticky top-0 z-50 shadow-sm transition-colors duration-500">
        <div className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter cursor-pointer" onClick={() => navigate('/buyer-portal')}>
          🌾 Farm<span className="text-[#fbbc05]">2</span>Home
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/buyer-portal')} className="px-6 py-2.5 rounded-full border border-gray-100 dark:border-white/10 text-gray-400 dark:text-gray-500 text-[10px] font-black uppercase tracking-widest hover:text-gray-900 dark:hover:text-white hover:border-gray-900 dark:hover:border-white transition-all font-bold">
            ← DASHBOARD
          </button>
        </div>
      </nav>
      ...
      <div className="flex-1 max-w-5xl w-full mx-auto px-6 py-12">
        <div className="mb-12">
          <h1 className="text-5xl font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none">{t('settings.security')}</h1>
          <p className="text-gray-400 dark:text-gray-500 font-bold text-xs mt-3 uppercase tracking-[0.4em] flex items-center gap-3 px-1">
             Manage Consumer Credentials <span className="w-2 h-2 rounded-full bg-[#fbbc05] animate-pulse inline-block"></span>
          </p>
        </div>

        {/* Language Preference Section */}
        <div className="mt-10 bg-white dark:bg-[#111111] border border-gray-100 dark:border-white/5 rounded-[3rem] p-10 shadow-sm transition-colors duration-500">
          <div className="mb-6">
            <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">{t('settings.language')}</h2>
            <p className="text-gray-400 dark:text-gray-500 text-xs mt-2 font-medium">{t('settings.languageDesc')}</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              >
                ...
                <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${language === code ? 'text-[#fbbc05]' : 'text-gray-300 dark:text-gray-600'}`}>{sub}</p>
        ...
        <div className="bg-white dark:bg-[#111111] border border-gray-100 dark:border-white/5 rounded-[4rem] p-12 shadow-2xl shadow-gray-900/5 relative overflow-hidden group transition-colors duration-500">
           <div className="absolute top-0 left-0 w-full h-40 bg-gradient-to-b from-amber-50/30 dark:from-amber-400/5 to-transparent"></div>
           
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10 mb-16">
             <div className="flex items-center gap-8">
                <div className="w-32 h-32 shrink-0 rounded-[2.5rem] overflow-hidden border-4 border-white dark:border-gray-800 bg-gray-50 dark:bg-white/10 flex items-center justify-center font-black text-4xl text-gray-900 dark:text-white shadow-2xl shadow-amber-900/10 transition-transform duration-500 group-hover:scale-105">
                ...
                <div>
                  ...
                  <div className="inline-flex px-4 py-1.5 bg-amber-50 dark:bg-amber-400/10 border border-amber-100 dark:border-amber-400/20 text-[#fbbc05] text-[10px] font-black uppercase tracking-widest rounded-full shadow-sm">
              ...
              <button
                ...
                className="px-10 py-5 bg-gray-900 dark:bg-white text-white dark:text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl hover:bg-[#fbbc05] transition-all shadow-xl shadow-gray-900/10 hover:-translate-y-1 active:scale-95 duration-300"
              >
               Modify Profile
             </button>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10 border-t border-gray-50 dark:border-white/5 pt-12">
              ...
              <div className="bg-gray-50/50 dark:bg-white/5 p-8 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-inner">
                <div className="text-[10px] text-gray-400 dark:text-gray-500 font-black uppercase tracking-[0.4em] mb-3 px-1">Access Protocol</div>
                <div className="text-xl text-gray-900 dark:text-white font-black tracking-tighter leading-none mb-2">Encrypted Password</div>
                <div className="text-[10px] text-[#fbbc05] font-black uppercase tracking-widest">• • • • • • • •</div>
              </div>
            </div>
         </div>

        <div className="mt-10 bg-red-50/30 dark:bg-red-950/10 border border-red-50 dark:border-red-900/20 rounded-[3rem] p-10 flex flex-col lg:flex-row items-center justify-between gap-8 transition-all hover:bg-red-50/50">
          <div className="text-center lg:text-left">
            <h2 className="text-xl font-black text-red-500 dark:text-red-400 uppercase tracking-tighter">Decommission Account</h2>
            <p className="text-gray-400 dark:text-gray-500 text-xs mt-2 font-medium italic">Permanently erase your consumer profile and purchase history from the platform.</p>
          </div>
          {!showDeleteConfirm ? (
            <button 
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto animate-in slide-in-from-right duration-500">
               ...
               <div className="flex gap-3">
                   <button onClick={() => setShowDeleteConfirm(false)} className="px-6 py-3 rounded-2xl bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 text-gray-400 dark:text-gray-500 text-[10px] font-black uppercase tracking-widest hover:text-gray-900 dark:hover:text-white transition-all shadow-sm">Cancel</button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-10">
          <div className="absolute inset-0 bg-white/40 dark:bg-black/40 backdrop-blur-md" onClick={() => setIsModalOpen(false)}></div>
          <div className="relative w-full max-w-4xl bg-white dark:bg-[#111111] border border-gray-100 dark:border-white/5 rounded-[4rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in duration-300">
            ...
            <div className="p-10 md:p-16 overflow-y-auto flex-1 no-scrollbar space-y-12">
              {status.message && (
                <div className={`p-6 rounded-[2rem] border transition-all animate-in slide-in-from-top duration-500 font-black text-xs uppercase tracking-widest ${
                  status.type === 'error' ? 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-900/20 text-red-500 dark:text-red-400' : 'bg-amber-50 dark:bg-amber-400/10 border-amber-100 dark:border-amber-400/20 text-[#fbbc05]'
                }`}>
                  {status.message}
                </div>
              )}
              ...
              <form id="updateProfileForm" onSubmit={handleSubmit} className="space-y-16">
                 <div>
                  <h3 className="text-[11px] font-black text-[#fbbc05] uppercase tracking-[0.5em] mb-8 border-b border-amber-50 dark:border-amber-400/10 pb-4">Visualization</h3>
                  <div className="flex items-center gap-10">
                    <div className="relative group">
                      <div className="w-32 h-32 rounded-[2.5rem] overflow-hidden border-4 border-gray-50 dark:border-gray-800 bg-gray-100 dark:bg-white/10 flex items-center justify-center font-black text-4xl text-gray-300 dark:text-gray-600 shadow-xl transition-all duration-500 group-hover:shadow-[#fbbc05]/20">
                        {avatarPreview ? <img src={avatarPreview} alt="Avatar" className="w-full h-full object-cover" /> : initials}
                      </div>
                      <button type="button" onClick={() => avatarInputRef.current?.click()} className="absolute -bottom-2 -right-2 w-10 h-10 rounded-full bg-gray-900 border-4 border-white dark:border-gray-800 flex items-center justify-center text-white hover:bg-[#fbbc05] transition-all text-sm shadow-xl">✎</button>
                      <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={handleAvatarChange} />
                    </div>
                    <div className="flex-1">
                      <div className="text-gray-400 dark:text-gray-500 text-sm font-medium italic opacity-60">"Your profile visualization and name will be shared with farmers during order processing."</div>
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="text-[11px] font-black text-[#fbbc05] uppercase tracking-[0.5em] mb-8 border-b border-amber-50 dark:border-amber-400/10 pb-4">Personal Identity</h3>
                ...
                <div>
                  <h3 className="text-[11px] font-black text-[#fbbc05] uppercase tracking-[0.5em] mb-8 border-b border-amber-50 dark:border-amber-400/10 pb-4">Security Protocol</h3>
            ...
            <div className="p-10 border-t border-gray-50 dark:border-white/5 bg-white dark:bg-[#111111] sticky bottom-0 z-10 flex gap-6 justify-end">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-10 py-5 rounded-2xl border border-gray-100 dark:border-white/10 text-gray-400 dark:text-gray-500 text-[10px] font-black uppercase tracking-widest hover:text-gray-900 dark:hover:text-white transition-all font-bold">Cancel</button>
              <button form="updateProfileForm" type="submit" disabled={loading} className="px-12 py-5 rounded-2xl bg-gray-900 dark:bg-white text-white dark:text-black text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-gray-900/10 hover:bg-[#fbbc05] hover:-translate-y-1 transition-all duration-300 disabled:opacity-50">
